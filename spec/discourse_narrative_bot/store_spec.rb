...
describe DiscourseNarrativeBot::Store do
  describe '.set' do
    it 'should set the right value in the plugin store' do
      described_class.set('test', 10, 'yay')
      ...
      expect(plugin_store_row.key).to eq(described_class.key('test', 10))
    end
  end
  ...
  describe '.get' do
    it 'should get the right value from the plugin store' do
      PluginStoreRow.create!(
        ...
        key: described_class.key('test', 10),
        value: 'yay',
        type_name: 'string'
      )

      expect(described_class.get('test', 10)).to eq('yay')
    end
  end
end
